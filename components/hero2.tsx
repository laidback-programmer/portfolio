...
const SOCIALS = [
  ...
  {
    icon: LinkedinIcon,
    href: "https://www.linkedin.com/in/arya-kant-rajvanshi-141bbb28",
    label: "LinkedIn",
    external: true,
      ...
      <div className="mt-6 flex items-center gap-4 pb-4">
        {SOCIALS.map(({ icon: Icon, href, label, external, hoverClass }) => (
          <div key={label} className="relative">
            ...
            {hoveredLabel === label && (
              <div className="absolute left-1/2 top-full z-40 mt-2 -translate-x-1/2 whitespace-nowrap rounded-md bg-black/70 px-2 py-1 text-xs font-medium text-gray-200">
                {label}
              </div>
            )}
          </div>
